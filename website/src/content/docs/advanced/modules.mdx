...
Exporting means, which symbols (functions, variables, types) are accessible from outside the module. The public symbols are usually written in UpperCase, while private symbols are in lowercase. We follow this convention to differentiate between public and private symbol_table.
```ferret
