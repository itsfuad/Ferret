...
description: "Phase graphs, shared context, CFG reasoning, and runtime integration for Ferret's .fer -> C -> binary pipeline."
pubDate: 2025-12-18
...
- How CFG/analysis fits into the pipeline.
...
compiler: {
  ...
  phase4_typecheck.typedast -> phase5_cfg.cfganalyzer: "typed AST"
  phase5_cfg.proceedcodegen -> phase6_codegen.codegen: "CFG data"
  phase6_codegen.ccode -> runtime -> build.cc -> build.linker -> binary
  ...
  context -> phase5_cfg: "typed AST"
  context -> phase6_codegen: "CFG data"
  ...
  phase5_cfg.cfganalyzer -> context.diagnostics: "CFG errors"
  ...
  # Runtime integration
  phase6_codegen.includes.runtime -> runtime.headers: "includes"
...
- **Codegen gate**: Code generation is skipped when CFG analysis finds errors.
...
- **Narrow phases**: Each stage does one job (collect symbols, check types, build CFG) and doesn't redo previous work.
