...
// Union-based Print/Println for Printable union
// The union layout is: [4-byte tag][variant data]
// Slice layout: [pointer to data][length][capacity]
void ferret_io_Println_slice(const void* data_ptr, int64_t len, int64_t cap);
void ferret_io_Print_slice(const void* data_ptr, int64_t len, int64_t cap);

// Single value variants (for compatibility)
void ferret_io_Println_union(const void* union_ptr);
void ferret_io_Print_union(const void* union_ptr);
